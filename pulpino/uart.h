...
public:
    std::array<scc::bitfield_register<uint32_t>, 8> registers{{
        {"RBR_THR_DLL", 0x0000},
        ///< Receiver Buffer Register (RBR if DLAB=0 && read), Transmitter Holding Register (THR if DLAB=0 && write),
        ///< Divisor Latch (LS) (if DLAB=1)
        {"IER_DLM", 0x0004},
    ...
    /////////////////////////////////////////////////////////////////////////////
    /// \brief Constructor
    UARTRegs(sc_core::sc_module_name name)
    : tlm_target_bfs_register_base<UARTRegs>{name} {};
};

/////////////////////////////////////////////////////////////////////////////
/// \class UART
/// \brief tlm_target_bfs based UART peripheral. Losely follows 16650 standard
template <typename owner_t> class UART : public scc::tlm_target_bfs<UARTRegs, owner_t> {
...
inline UART<owner_t>::UART(sc_core::sc_module_name name, scc::tlm_target_bfs_params&& params, owner_t* owner)
...
template <class owner_t> void UART<owner_t>::b_transport_ext(tlm::tlm_generic_payload& gp, sc_core::sc_time& delay) {
...
template <class owner_t> void UART<owner_t>::reset() {
    for(auto& bf : bfs_t::regs->bitfields) {
        bf = 0;
    }
}

template <class owner_t> void UART<owner_t>::sendData(uint8_t data) {
...
template <class owner_t> void UART<owner_t>::getData(uint8_t& data) {
...
template <class owner_t> void UART<owner_t>::send_interrupt_process(void) {
    (*bfs_t::irq_out_)[0] = 0;
    while(1) {
