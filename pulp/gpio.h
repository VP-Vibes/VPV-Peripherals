...
class GPIORegs : public scc::tlm_target_bfs_register_base<GPIORegs> {
 public:
  std::array<scc::bitfield_register<uint32_t>, 6 + 8> registers{
    ...
    {"INTTYPE", 0x10},
    {"INTSTATUS", 0x18, 0, 0},
    REGISTER_ARRAY("PADCFG_reg", 0x20, 4, 8)
  }}; ///< GPIO registers
  std::array<scc::bitfield<uint32_t>, 6 * 32 + 1> bitfields{
    ...
    {getRegister("PADCFG_reg7"), "PADCFG31", 24, 5, "gpio.pads_31.cfg"}
...
/////////////////////////////////////////////////////////////////////////////
/// \class GPIO
/// \brief tlm_target_bfs based GPIO peripheral. Losely follows Pulp platform APB GPIO: https://github.com/pulp-platform/apb_gpio
template <typename owner_t>
class GPIO : public scc::tlm_target_bfs<GPIORegs, owner_t> {
...
template <class owner_t>
void GPIO<owner_t>::setupPadout(void) {
  for (size_t i = 0; i < 32; i++) {
...
template <class owner_t>
void GPIO<owner_t>::setupPaddir(void) {
  for (size_t i = 0; i < 32; i++) {
...
template <class owner_t>
void GPIO<owner_t>::setupIntstatus(void) {
...
template <class owner_t>
void GPIO<owner_t>::intstatus_cb(void) {
  for (size_t i = 0; i < 32; i++) {
