...
public:
  std::array<scc::bitfield_register<uint32_t>, 32 + 1 + 3*4> registers{{
    {"PRIO_SOURCE0",        0x0000}, // RESERVED, i.e. Priority interrupt source 0 does not exist. No source, or source 0 ID, means no interrupt
    ...
    /* Context-specific priority threshold: The Plic will permit all interrupts which have a priority of less or equal than the value. To the respective context */
...
///\details only up to 32 sources [31:0] for 4 [3:0] targets supported. Limited due to scc::tlm_target_bfs_register_base not supporting dynamic construction of registers (move=)
template<typename owner_t, size_t N_TARGET, size_t N_SOURCE, uint32_t LE_SRC_MASK=0, size_t N_LINEREGS = (N_SOURCE>>5)+((N_SOURCE%32)? 1: 0)>
...
public:
  ...
  std::array<scc::bitfield<uint32_t>*, N_TARGET> ccs_{};
    ///< claim/complete register. A read to this register returns the id of the (highest prio) interrupt. A write with enabled bits resets (=completes) the respective pending interrupts
...
inline Plic<owner_t, N_TARGET, N_SOURCE, LE_SRC_MASK, N_LINEREGS>::Plic(sc_core::sc_module_name name, scc::tlm_target_bfs_params&& params, owner_t* owner)
  : bfs_t(name, std::move(params), owner)
{
  ...
  for(size_t i = 0; i< N_TARGET; ++i) {
    targets_[i] = std::make_unique<Plic_target>(
      *(ips_[0]),
      *(ies_[i]),
      *(pths_[i]),
      prios_
  ...
  for(size_t i = 1; i< N_SOURCE; ++i) {
    ...
    prios_[i]->setReadCallback([this, i](auto&&) {
      return (prios_[i]->get());
    });
  }
  // -------------------------------------------------------------------------------
  for(size_t i = 0; i < N_LINEREGS; ++i) {
    ips_[i]->setWriteCallback([this, i](auto&&, uint32_t& valueToWrite) {
      *(ips_[i]) = valueToWrite;
    });
    ips_[i]->setReadCallback([this, i](auto&&) {
      return (ips_[i]->get());
    });
  }
  // -------------------------------------------------------------------------------
  for(size_t i = 0; i < N_TARGET; ++i) {
    ...
    ies_[i]->setReadCallback([this, i](auto&&) {
      return (ies_[i]->get());
    });
  }
  // -------------------------------------------------------------------------------
